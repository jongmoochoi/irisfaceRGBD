...
void setConfigAll(const json::Object& config)
{
	...
	for(int i=0; i<omglob.numInputLdmks; i++) {
		...
		omglob.inputOuterLdmk3DPts.push_back(Point3f(x, y, z));
	}
	...
	for(int i=0; i<omglob.numInputLdmks; i++) {
		...
		omglob.inputInnerLdmk3DPts.push_back(Point3f(x, y, z));
	}
...
int getFaceType(const vector<Point2f>& imgPts)
{
	...
	float left2midDis = norm(imgPts[leftLdmkId] - imgPts[midLdmkId]);
	float right2midDis = norm(imgPts[rightLdmkId] - imgPts[midLdmkId]);
	float left2rightDis = norm(imgPts[leftLdmkId] - imgPts[rightLdmkId]);

	float epsilon = 0.75;	// empirical

	// if M is between L and R
	if(left2rightDis > left2midDis && left2rightDis > right2midDis) {
		// if ratio is near frontal
		float ratio = (left2midDis > right2midDis) ? (right2midDis/left2midDis) : (left2midDis/right2midDis);
		if(ratio > epsilon) {
