...
public:
	...
	int leftRigidLdmkId, midRigidLdmkId, rightRigidLdmkId; // ids based on the rigid ids [0, numRigidLdmks-1]
	map<int, int> ldmkIdPairLookupTable;
	vector<int> inputLdmksKeyTable, inputLdmksRigidTable;	// for looking up by ldmk_id
	vector<int> ldmkTypeTable; // look up landmark type (left, right, middle)
	vector<bool> sampleTable, segmentTable;
	float modelFaceWidth;
	
	// utils
	vector<string> fileList;	//only filename with no extension
	string inputDir, outputDir;

	vector<Point3f> inputOuterLdmk3DPts, inputInnerLdmk3DPts;
